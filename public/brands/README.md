...
`data/brands.json` records source pages, original asset URLs, retrieval timestamps and SHA-256 hashes. `data/brand-sites.json` maps 29 companies to 33 drug–indication records. Nuvalent and GSK both appear for their documented relationship.
...
Scholar Rock's SVG is returned inline by Firecrawl. Praxis, Scholar Rock, Nuvalent and Mineralys have white wordmarks, so the interface places them on a dark surface. GSK's static official mark is used instead of its animated homepage logo. Corcept and Exelixis use their symbol alongside the company name. Vanda's official logo supplies its compact mark where branding extraction returned no separate favicon. Ultragenyx's main-site assets denied direct retrieval; its investor-site branding supplies the official icon used here. PolyPid's legacy Adobe XML namespace declarations are expanded and its document type removed; paths and colors remain unchanged. No logos were invented, recolored or reconstructed.
