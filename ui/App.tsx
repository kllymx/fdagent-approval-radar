...
function CompanyLogos({ candidateId, compact = false }: { candidateId: string; compact?: boolean }) {
  ...
  return <div className={`company-logos ${compact ? 'compact' : ''}`} aria-label={compact ? undefined : 'Sponsor companies'}>{(compact ? companies.slice(0, 1) : companies).map((company) => compact ? <CompanyMark company={company} compact key={company.id} /> : <a href={safeSourceUrl(company.website)} target="_blank" rel="noreferrer" key={company.id} aria-label={company.name}><CompanyMark company={company} compact={false} />{(company as Company & { displayKind?: string }).displayKind === 'symbol' && <span className="company-name">{company.name}</span>}</a>)}</div>;
}
...
function ResearchDesk({ candidate, sources, onOpenSources, runtime, onInvestigate, busy, onAnalysis, hasInvestigation, model, onValidation, savedRun, onLoadRun }: {
  ...
  return <div className="research-grid">
    <div className="research-main">
      <section className="section dossier"><p className="dossier-summary">{candidate.summary}</p></section>
      <section className="section timeline-section"><div className="section-heading"><h2>Review timeline</h2></div>
    ...
    <aside className="research-aside">
      ...
      <section className="section signals-section"><div className="section-heading"><h2>What matters</h2></div>
        {candidate.signals.map((signal) => <details className={`signal ${signal.direction}`} key={signal.id} open={signal.direction === 'concern'}><summary><DirectionIcon direction={signal.direction} /><h3>{signal.label}</h3><ChevronDown size={13} /></summary><div className="signal-detail"><p>{signal.detail}</p><Sources ids={signal.sourceIds} sources={sources} onOpen={onOpenSources} /></div></details>)}
        {!candidate.signals.length && <p className="muted padded">No candidate-specific signals are included yet.</p>}
      </section>
      <p className="quiet-note">Public disclosures can be incomplete. Missing evidence remains an open question.</p>
    </aside>
  </div>;
}
