...
export default function AssessmentView({ candidate, assessment, onSources, onInvestigation }: { candidate: Candidate; assessment?: Assessment; onSources: (ids: string[]) => void; onInvestigation: () => void }) {
  ...
  if (action.completed) return <section className="candidate-assessment completed-assessment"><OutlookBadge category={category} /><p>{action.date ? `${action.label} on ${formatDate(action.date)}.` : 'This review episode has a completed regulatory action.'} The timeline preserves the review history.</p></section>;
  ...
  return <section className={`candidate-assessment assessment-${category}`} aria-label="Astra evidence assessment">
    <div className="assessment-heading"><div><OutlookBadge category={category} /><span className="assessment-scope">{scopeLabel}</span></div><button className="text-button" onClick={onInvestigation}>{assessment.scope === 'investigation' ? 'Open investigation' : 'Explore with Astra'}<ArrowRight size={13} /></button></div>
    ...
    <p className="assessment-crux"><span>Pivotal question</span>{assessment.pivotalQuestion}</p>
