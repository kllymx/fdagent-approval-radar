...
import type { Candidate, Dashboard, Source } from './types';
...
export function filterCandidates(candidates: Candidate[], filters: { query: string; status: string; month: string; sponsor: string }): Candidate[] {
...
export default function CandidateOverview({ dashboard, onSelect, onResume, selectedCandidate, renderLogo, onSources, extraSources }: {
  dashboard: Dashboard;
  onSelect: (id: string, analysis?: boolean) => void;
  onResume: () => void;
  ...
  const filtered = useMemo(() => filterCandidates(candidates, { query, status, month, sponsor }).sort((a, b) => {
  ...
  }), [candidates, query, status, month, sponsor, sort, extraSources]);
  const hasFilters = Boolean(query || status || month || sponsor);
  const clearFilters = () => { setQuery(''); setStatus(''); setMonth(''); setSponsor(''); };
  return <section className="candidate-overview">
    <div className="overview-page-heading"><div><h1>All candidates</h1><p>Explore review timelines and public evidence across drugs and indications.</p></div>{selectedCandidate && <button className="text-button resume-candidate" onClick={onResume}>Return to {selectedCandidate.drug}<ArrowRight size={14} /></button>}</div>
    ...
    <div className="catalog-table-wrap"><table className="catalog-table"><thead><tr><th scope="col">Drug and indication</th><th scope="col">Sponsor</th><th scope="col">Review</th><th scope="col">FDA action / target</th><th scope="col">Evidence</th><th scope="col"><span className="sr-only">Open candidate</span></th></tr></thead><tbody>{filtered.map((candidate) => {
