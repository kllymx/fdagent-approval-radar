...
export default function CandidateOverview({ dashboard, onSelect, onResume, selectedCandidate, renderLogo, onSources, extraSources }: {
  ...
  const investigatedCount = new Set(dashboard.investigations.filter((run) => candidates.some((candidate) => candidate.id === run.candidateId)).map((run) => run.candidateId)).size;
