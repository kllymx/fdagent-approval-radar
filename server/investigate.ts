...
- Safety counts must retain their event definition and denominator: a registry's other/nonserious adverse-event table is not a total treatment-emergent adverse-event count, and a reporting threshold may omit less frequent events. Keep serious events, severity grades, treatment attribution, discontinuations and deaths separate. If a long paper or presentation is truncated before its safety table, use find_in_source for relevant safety, Grade, discontinuation or dose-modification terms before declaring those data unavailable.
